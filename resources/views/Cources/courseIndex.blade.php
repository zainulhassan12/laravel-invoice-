           ...
           @if (isset($item->price))
            ...
            <div class="border-left-span"></div>
              <a id="modal-button" href="" data-toggle="modal" data-cid="{{$item->id}}" data-course="{{$item->fullname}}" data-drop="{{$price}}" data-selected="{{$item->price}}" data-target="#addPrice" class="badge bg" style="">
                <i class="bi bi-file-earmark-plus" style="color:darkcyan;font-size:19px" title="Add Prices"></i>
              </a> 
            <div class="border-left-span"></div>
              <a href="{{Route('PriceIndex')}}" class="badge bg" style="">
                <i class="fa fa-cogs" style="color:deepskyblue;font-size:19px" title="Manage Prices"></i>
              </a>   
        </td> 
        <td>
...
$(document).ready(function(){
  ...
  var modal = $(this);
  $('#priceForm').attr('action', '/cources/addPriceCourse/'+cid)
